...
import { Home, Search, Bookmark, CheckSquare, User, Menu, X } from "lucide-react";
...
export function TopNavigation() {
  ...
  return <nav className="bg-canvas-cloud border-b border-accent-lavender/30 bg-[#f0f1f3]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 bg-[#f4f5f0] border-none">
        <div className="flex items-center justify-between h-16 border-none">
          ...
          <div className="hidden md:flex items-center space-x-4 lg:space-x-6 ml-auto">
            ...
            return <Link key={item.title} to={item.url} className={cn("flex items-center gap-2 px-3 py-2 text-sm font-medium transition-colors rounded-md", isActive ? 'bg-white text-ink-obsidian shadow-sm' : 'text-ink-obsidian/80 hover:bg-white hover:shadow-sm')}>
