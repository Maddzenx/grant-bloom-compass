...
const GrantCard = ({
  ...
  return <Card className={`p-4 cursor-pointer transition-all duration-200 border-l-4 ${isSelected ? 'bg-accent-lavender/10 border-l-accent-lavender shadow-md' : 'bg-white border-l-transparent hover:bg-accent-lavender/5 hover:shadow-sm'} ${isMobile ? 'mx-2' : 'mx-1'}`} onClick={onSelect}>
      <div className="space-y-3">
        ...
        <div className="flex items-start justify-between">
          ...
          onToggleBookmark();
            ...
            <Bookmark className={`w-4 h-4 ${isBookmarked ? 'fill-accent-lavender text-accent-lavender' : 'text-ink-obsidian/40'}`} />
          </button>
        </div>
