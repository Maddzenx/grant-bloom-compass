...
const FilterContent = ({ filters, onFiltersChange, onResetFilters, organizationOptions, fundingRange, onFundingRangeChange, deadlineValue, onDeadlineChange, industryOptions, eligibleApplicantOptions, geographicScopeOptions, totalGrantsCount }) => (
  <>
    <div className="flex-1 overflow-y-auto p-6 space-y-8">
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          {filters.organizations?.length > 0 && (
            <Button variant="link" size="sm" className="p-0 h-auto text-purple-600 font-semibold" onClick={() => onFiltersChange({ organizations: [] })}>
              Återställ
            </Button>
          )}
        </div>
        <div className="space-y-4">
          {organizationOptions.map(org => (
            <label key={org} className="flex items-center gap-3 cursor-pointer text-base text-ink-secondary">
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
            ...
            {(fundingRange.min !== null || fundingRange.max !== null) && (
                 <Button variant="link" size="sm" className="p-0 h-auto text-purple-600 font-semibold" onClick={() => onFundingRangeChange({ min: null, max: null })}>
                    Återställ
                </Button>
            )}
        </div>
        <div className="flex items-center gap-4">
          <div className="flex-1">
            ...
            <input type="number" className="w-full bg-gray-100 border-transparent rounded-md text-base p-2" value={fundingRange.min ?? ''} onChange={e => onFundingRangeChange({ ...fundingRange, min: e.target.value ? Number(e.target.value) : null })} placeholder="0"/>
          </div>
          <div className="flex-1">
            <label className="text-sm font-medium text-gray-500 mb-1 block">Högst</label>
            <input type="number" className="w-full bg-gray-100 border-transparent rounded-md text-base p-2" value={fundingRange.max ?? ''} onChange={e => onFundingRangeChange({ ...fundingRange, max: e.target.value ? Number(e.target.value) : null })} placeholder="Ingen gräns"/>
          </div>
        </div>
      </div>
      <Separator className="bg-purple-200"/>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          {deadlineValue?.preset && (
            <Button variant="link" size="sm" className="p-0 h-auto text-purple-600 font-semibold" onClick={() => onDeadlineChange({ type: 'preset', preset: '' })}>
              Återställ
            </Button>
          )}
        </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          {filters.industrySectors?.length > 0 && (
            <Button variant="link" size="sm" className="p-0 h-auto text-purple-600 font-semibold" onClick={() => onFiltersChange({ industrySectors: [] })}>
              Återställ
            </Button>
          )}
        </div>
        <div className="space-y-4">
          {industryOptions.map(ind => (
            <label key={ind} className="flex items-center gap-3 cursor-pointer text-base text-ink-secondary">
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          {filters.eligibleApplicants?.length > 0 && (
            <Button variant="link" size="sm" className="p-0 h-auto text-purple-600 font-semibold" onClick={() => onFiltersChange({ eligibleApplicants: [] })}>
              Återställ
            </Button>
          )}
        </div>
        <div className="space-y-4">
          {eligibleApplicantOptions.map(app => (
            <label key={app} className="flex items-center gap-3 cursor-pointer text-base text-ink-secondary">
      ...
      <div className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            {(filters.consortiumRequired || filters.cofinancingRequired) && (
              <Button variant="link" size="sm" className="p-0 h-auto text-purple-600 font-semibold" onClick={() => onFiltersChange({ consortiumRequired: null, cofinancingRequired: null })}>
                Återställ
              </Button>
            )}
          </div>
          <label className="flex items-center gap-3 cursor-pointer text-base text-ink-secondary">
              <input type="checkbox" className="h-5 w-5 rounded border-gray-300 text-purple-600 focus:ring-purple-500" checked={filters.consortiumRequired === true} onChange={e => onFiltersChange({ consortiumRequired: e.target.checked ? true : null })}/>
              <span>Kräver consortium</span>
          </label>
           <label className="flex items-center gap-3 cursor-pointer text-base text-ink-secondary">
              <input type="checkbox" className="h-5 w-5 rounded border-gray-300 text-purple-600 focus:ring-purple-500" checked={filters.cofinancingRequired === true} onChange={e => onFiltersChange({ cofinancingRequired: e.target.checked ? true : null })}/>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          {filters.geographicScope?.length > 0 && (
            <Button variant="link" size="sm" className="p-0 h-auto text-purple-600 font-semibold" onClick={() => onFiltersChange({ geographicScope: [] })}>
              Återställ
            </Button>
          )}
        </div>
        <div className="space-y-4">
          {geographicScopeOptions.map(scope => (
            <label key={scope} className="flex items-center gap-3 cursor-pointer text-base text-ink-secondary">
...
export const FilterBar: React.FC<FilterBarProps> = (props) => {
  ...
  const activeFilterCount = [
    ...
    props.filters.geographicScope?.length,
  ].filter(Boolean).reduce((acc: number, count: any) => acc + (typeof count === 'number' ? count : 0), 0);

  const TriggerButton = (
    <Button variant="outline" className="flex items-center gap-2 rounded-full px-3 py-1 bg-white border-gray-300 text-ink-obsidian font-medium text-xs shadow-none hover:bg-gray-50 min-h-0 h-7">
      ...
      <div className="flex flex-row items-center gap-2 mt-0 mb-0 overflow-x-auto pb-2 scrollbar-hide">
        ...
        <Popover>
            ...
            <PopoverContent className="w-56 p-3" align="start">
                {organizationOptions.map(org => (
                    <label key={org} className="flex items-center gap-2 cursor-pointer text-sm font-medium text-gray-700">
        ...
        <Popover>
            ...
            <PopoverContent className="w-56 p-3" align="start">
                <select className="w-full border-gray-300 rounded-md shadow-sm text-sm" value={deadlineValue.preset || ''} onChange={e => onDeadlineChange({ type: 'preset', preset: e.target.value })}>
                  <option value="">Alla</option>
                  <option value="urgent">Brådskande (7 dagar)</option>
                  <option value="2weeks">Nästa 2 veckor</option>
                  <option value="1month">Nästa månad</option>
        ...
        <Popover>
            ...
            <PopoverContent className="w-56 p-3" align="start">
                <div className="space-y-3">
                  {industryOptions.map(ind => (
                    <label key={ind} className="flex items-center gap-2 cursor-pointer text-sm font-medium text-gray-700">
        ...
        <Popover>
            ...
            <PopoverContent className="w-56 p-3" align="start">
                <div className="space-y-3">
                  {eligibleApplicantOptions.map(app => (
                    <label key={app} className="flex items-center gap-2 cursor-pointer text-sm font-medium text-gray-700">
            ...
            <PopoverContent className="w-56 p-3" align="start">
                <div className="space-y-4">
                  <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-gray-700">
                      <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" checked={filters.consortiumRequired === true} onChange={e => onFiltersChange({ consortiumRequired: e.target.checked ? true : null })}/>
                      <span>Kräver consortium</span>
                  </label>
                   <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-gray-700">
                      <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" checked={filters.cofinancingRequired === true} onChange={e => onFiltersChange({ cofinancingRequired: e.target.checked ? true : null })}/>
                      <span>Kräver medfinansiering</span>
                  </label>
                </div>
            </PopoverContent>
        ...
        <Popover>
            ...
            <PopoverContent className="w-56 p-3" align="start">
                <div className="space-y-3">
                  {geographicScopeOptions.map(scope => (
                    <label key={scope} className="flex items-center gap-2 cursor-pointer text-sm font-medium text-gray-700">
