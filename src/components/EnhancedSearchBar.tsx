    ...
    <div className="relative w-full">
      <div className="relative">
        ...
        <Input
          ...
          className="pl-12 pr-10 py-3 border-gray-300 bg-white rounded-xl text-base font-medium shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-full"
        />
        {searchTerm && (
          <Button
            variant="ghost"
              ...
              {recentSearches
                ...
                .map((recent, index) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded hover:bg-gray-50 flex items-center gap-2 ${
                      index + suggestions.length === selectedSuggestionIndex ? 'bg-blue-50 text-blue-700' : ''
                    }`}
                  >
      ...
      {searchMetrics && process.env.NODE_ENV === 'development' && (
        <div className="absolute top-full left-0 mt-2 text-xs text-gray-500 bg-gray-50 px-2 py-1 rounded">
          {searchMetrics.resultsCount} resultat • {searchMetrics.searchLatency.toFixed(1)}ms
          {searchMetrics.cacheHitRate > 0 && ` • Cache: ${(searchMetrics.cacheHitRate * 100).toFixed(0)}%`}
        </div>
      )}
    </div>
  );
