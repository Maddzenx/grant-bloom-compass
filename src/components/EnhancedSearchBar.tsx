...
const EnhancedSearchBar = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    const allSuggestions = [...suggestions, ...recentSearches].filter(s => 
  ...
  const clearSearch = () => {
    console.log('🧹 Clearing search - before:', searchTerm);
    onSearchChange('');
    setShowSuggestions(false);
    setSelectedSuggestionIndex(-1);
    ...
    setTimeout(() => {
      if (inputRef.current) {
        inputRef.current.focus();
        console.log('🧹 Clearing search - after focus, value:', inputRef.current.value);
      }
    }, 0);
  };
    ...
    <div className="relative w-full">
      <div className="relative">
        <Search className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 ${
          isSearching ? 'text-gray-500 animate-pulse' : 'text-gray-400'
        }`} />
        <Input
          ref={inputRef}
          ...
          className="pl-12 pr-10 border-gray-300 bg-white rounded-xl text-base font-medium shadow-sm focus:ring-2 focus:ring-gray-500 focus:border-gray-500 w-full text-black placeholder:text-black py-px px-[49px]"
        />
        {searchTerm && (
          <Button
            variant="ghost"
        ...
        >
          ...
          {suggestions.length > 0 && (
            <div className="p-2">
              ...
              {suggestions
                .filter(s => s.toLowerCase().includes(searchTerm.toLowerCase()) && s !== searchTerm)
                .map((suggestion, index) => (
                  <button
                    key={`suggestion-${suggestion}`}
                    onClick={() => handleSuggestionClick(suggestion)}
                    className={`w-full text-left px-3 py-2 rounded hover:bg-gray-50 flex items-center gap-2 ${
                      index === selectedSuggestionIndex ? 'bg-gray-50 text-black' : ''
                    }`}
                  >
              ...
              {recentSearches
                ...
                .map((recent, index) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded hover:bg-gray-50 flex items-center gap-2 ${
                      index + suggestions.length === selectedSuggestionIndex ? 'bg-gray-50 text-black' : ''
                    }`}
                  >
      ...
      {searchMetrics && process.env.NODE_ENV === 'development' && (
        <div className="absolute top-full left-0 right-0 mt-1 p-2 bg-gray-100 text-xs text-gray-600 rounded border">
          Results: {searchMetrics.resultsCount} | 
          Latency: {searchMetrics.searchLatency.toFixed(0)}ms | 
          Cache: {(searchMetrics.cacheHitRate * 100).toFixed(0)}%
        </div>
      )}
    </div>
  );
