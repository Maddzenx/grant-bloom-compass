...
const EnhancedSearchBar = ({
  ...
  const isEnhancedQuery = searchTerm.length > 10 || /[^\w\s]/.test(searchTerm) || searchTerm.includes('?');
    ...
    <div className="relative w-full">
      <div className="relative">
        ...
        <Input
          ...
          className="pl-12 pr-10 border-gray-300 bg-white rounded-xl text-base font-medium shadow-sm focus:ring-2 focus:ring-gray-500 focus:border-gray-500 w-full text-black placeholder:text-black py-3"
        />
        {searchTerm && (
          <Button
            type="button"
      ...
      {/* Search metrics (only show in development) */}
      {searchMetrics && process.env.NODE_ENV === 'development' && (
        <div className="absolute top-full right-0 mt-1 text-xs text-gray-500 bg-white px-2 py-1 rounded border">
          {searchMetrics.resultsCount} results in {searchMetrics.searchLatency}ms
        </div>
      )}
    </div>
  );
