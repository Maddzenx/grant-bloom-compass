      ...
      <div className="w-full px-0 pt-0 pb-0">
        <div className="flex flex-col gap-3 mt-0 mb-2 w-full px-0">
          ...
          <div className="flex justify-center">
            <div className="flex items-center bg-white rounded-lg border border-zinc-200 p-1 shadow-sm" role="tablist" aria-label="Sökläge">
              <button onClick={handleToggleSearchMode} className={`flex items-center gap-2 ${baseBtn} rounded-md type-body font-medium transition-all duration-200 font-['Source_Sans_3'] ${!isAISearch ? 'bg-[#7D54F4] text-white shadow-sm' : 'text-zinc-600 hover:text-zinc-800 hover:bg-zinc-50'}`} title={isAISearch ? 'AI sökning' : 'Vanlig sökning'}>
          ...
          top: 'calc(env(safe-area-inset-top, 0px) + 0px)'
            ...
            <textarea ref={textareaRef} className="w-full pl-6 pr-24 py-4 rounded-lg border border-zinc-200 bg-white type-body font-medium text-zinc-900 focus:ring-2 focus:ring-zinc-300 focus:border-zinc-300 focus:outline-none placeholder:text-zinc-500 resize-none overflow-hidden font-['Source_Sans_3']" placeholder={isAISearch ? "Beskriv ditt projekt för AI-matchning..." : "Sök efter bidrag..."} value={searchTerm} onChange={e => {
            const maxChars = 300;
            if (e.target.value.length <= maxChars) {
              onSearchChange(e.target.value);
            }
          }} onFocus={() => setIsInputFocused(true)} onBlur={() => setIsInputFocused(false)} onKeyPress={handleKeyPress} onInput={e => {
            const textarea = e.target as HTMLTextAreaElement;
            textarea.style.height = 'auto';
            const newHeight = Math.min(textarea.scrollHeight, 180);
            textarea.style.height = newHeight + 'px';
          }} rows={1} maxLength={300} aria-label="Search grants" style={{
            minHeight: '56px',
            maxHeight: '180px'
          }} />
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2" role="group" aria-label="Sökåtgärder">
              ...
              <button className={`flex items-center gap-1 rounded-md ${isMobile ? 'px-3 py-2 min-h-[44px]' : 'p-2'} bg-[#7D54F4] hover:bg-[#6a40f2] text-white shadow-sm active:scale-[0.98]`} onClick={handleSearchClick} aria-label="Search">
          ...
          {!isInputFocused && <div className="relative mt-1 px-1 overflow-x-auto whitespace-nowrap [-webkit-overflow-scrolling:touch]">
              <div className="inline-flex gap-2 pr-4">
                {chipsToShow.map((s, idx) => <button key={idx} className="inline-flex type-secondary text-zinc-700 bg-white rounded-full px-3 py-1 transition-colors border-0 shadow-none hover:bg-zinc-50 hover:shadow-none focus:outline-none focus:ring-0" onClick={() => {
              onSearchChange(s);
              setTimeout(onSearch, 0);
            }} aria-label={`Sökförslag: ${s}`}>
                    {s}
                  </button>)}
                {defaultSuggestionChips.length > 3 && !showAllChips && <button className="inline-flex type-secondary text-zinc-700 bg-white rounded-full px-3 py-1 transition-colors border-0 shadow-none hover:bg-zinc-50 hover:shadow-none focus:outline-none focus:ring-0" onClick={() => setShowAllChips(true)} aria-label="Visa fler sökförslag">
                    Visa fler
                  </button>}
              </div>
            </div>}
