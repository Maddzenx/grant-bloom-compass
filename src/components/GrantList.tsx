...
const GrantList = ({
  ...
  return <div className={containerClass}>
      <ScrollArea className="flex-1">
        <div className="p-2 md:p-4 border border-transparent py-0 px-1 md:px-2 max-w-[400px] mx-auto block">
          <div className="space-y-2 md:space-y-3">
            {grants.length === 0 ? <div className="text-center text-gray-500 mt-8 px-4">
              ...
              </div> : grants.map(grant => <GrantCard key={grant.id} grant={grant} isSelected={selectedGrant?.id === grant.id} isBookmarked={bookmarkedGrants.has(grant.id)} isSeen={seenGrants.has(grant.id)} onSelect={() => onGrantSelect(grant)} onToggleBookmark={() => onToggleBookmark(grant.id)} isMobile={isMobile} />)}
          </div>
        </div>
      </ScrollArea>
    </div>;
